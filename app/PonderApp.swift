...
struct PonderApp: App {
	...
	var body: some Scene {
		WindowGroup {
			RootView()
				...
				.onAppear {
					PonderApp.sharedHomeViewModel = homeViewModel
					NotificationManager.shared.configure(servicesLocator: servicesLocator)
					NotificationCenter.default.post(name: .ponderAppReady, object: nil)
	...
	private func handleIncomingURL(_ url: URL) {
		if url.scheme == "ponder", url.host == "curatorVoice" {
...
extension Notification.Name {
	...
	static let ponderAppReady          = Notification.Name("PonderAppReady")
