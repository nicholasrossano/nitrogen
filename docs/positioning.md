...
Nitrogen AI is a **domain-agnostic, chat-first diligence reference architecture** with **climate-impact investment diligence and technical assistance** as the worked example. The repo is meant to be cloned, self-hosted, and extended; not a single-vendor SaaS lock-in.

## What changed (contract cutover)

The legacy **initiative** model, onboarding chat table, and `/initiatives/*` UI paths are retired (permanent redirect to `/projects/*`). **`projects`** is the canonical deal record; child tables scope to `project_id`. Personal work happens in **core chat**; shared curation crosses a single **promotion boundary** into project **findings** and structured **variables**. Legacy `/assumptions*` HTTP paths remain as aliases.
