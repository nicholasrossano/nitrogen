...
function itemTitle(item: BuildItem) {
  return (
    coerceDisplayString(item.content.title ?? item.content.name) ||
    coerceDisplayString(Object.values(item.content).find((v) => v != null)) ||
    'Item'
  );
}
...
function StaticRow({
  ...
  return (
    ...
    >
      <p className="text-sm font-medium text-text-primary leading-snug">{itemTitle(item)}</p>
    </ModuleItem>
  );
}
...
export function SimpleListView({ items, onDelete, onReorder, onAddToChat }: Props) {
  if (items.length === 0) {
    return <div className="py-8 text-center text-sm text-text-tertiary">No items yet.</div>;
  }
