...
interface AssessmentInstanceOpenDropdownProps {
  instances: AssessmentInstance[];
  onOpenInstance: (instance: AssessmentInstance) => Promise<void> | void;
  getInstanceLabel: (instance: AssessmentInstance) => string;
  buttonLabel?: string;
  menuClassName?: string;
  className?: string;
}

export function AssessmentInstanceOpenDropdown({
  instances,
  onOpenInstance,
  getInstanceLabel,
  buttonLabel = 'Open',
  ...
  className = '',
}: AssessmentInstanceOpenDropdownProps) {
    ...
    <div ref={pickerRef} className={`relative ${className}`}>
      ...
      {open ? (
        <div className={`absolute right-0 top-full mt-1 z-50 max-h-64 overflow-y-auto rounded-lg border border-divider bg-white py-1 shadow-lg ${menuClassName}`}>
          {sortedInstances.map((assessmentInstance) => {
            const openingThisInstance = openingInstanceId === assessmentInstance.id;
            const isApprovedInstance = assessmentInstance.is_plan_complete === true;
            const optionLabel = getInstanceLabel(assessmentInstance);
            return (
              <button
                key={assessmentInstance.id}
                type="button"
                disabled={openingThisInstance || Boolean(openingInstanceId)}
                onClick={async (event) => {
                  event.stopPropagation();
                  setOpeningInstanceId(assessmentInstance.id);
                  try {
                    await onOpenInstance(assessmentInstance);
                    setOpen(false);
                  } finally {
