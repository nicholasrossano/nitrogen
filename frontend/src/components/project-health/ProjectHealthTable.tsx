...
import { AlertTriangle, BookMarked, CheckCircle2, Database, FileText, HelpCircle, RefreshCw, Wrench, XCircle } from 'lucide-react';
import { api, type ProjectHealthAssessmentReference, type ProjectHealthResponse, type ProjectHealthSourceReference, type ProjectHealthStatus } from '@/lib/api';
...
function HealthSourcesMenu({
  sources,
  assessments,
  onOpenDocument,
  onOpenAssessment,
}: {
  sources: ProjectHealthSourceReference[];
  assessments: ProjectHealthAssessmentReference[];
...
export function ProjectHealthTable({
  ...
  const rows = useMemo(() => health?.dimensions ?? [], [health]);
  ...
  return (
    <div className="mt-2 rounded-xl border border-black/[0.05] bg-surface-subtle/40">
      ...
      {isLoading ? (
        <p className="px-4 py-4 text-sm text-text-tertiary">Generating health overview (this may take a few minutes)...</p>
      ) : (
        <div className="divide-y divide-divider">
          {rows.map((row) => {
            ...
            const sourceEntries = (row.retrieved_sources ?? [])
              .filter((src): src is ProjectHealthSourceReference => Boolean(src?.source_title && src?.source_type))
              .slice(0, 3);
            const assessmentEntries = (row.relevant_assessments ?? []).slice(0, 3);
            return (
              <div key={row.dimension_id} className="px-4 py-3">
                <div className="flex flex-col gap-2">
                  <div className="min-w-0">
                    <div className="flex items-start justify-between gap-3">
                      <p className="pt-1 text-[11px] font-medium uppercase tracking-[0.08em] text-text-secondary">
                        {row.label}
                      </p>
      ...
      {!hideRefreshButton ? (
        <div className="flex justify-end border-t border-divider px-4 py-3">
          <button
            ...
            title={readOnly ? 'View-only access cannot refresh health overview' : 'Refresh health overview'}
          >
