...
export function AssessmentWorkspace({
  ...
  const fetchState = useCallback(async () => {
    try {
      const data = await api.getStagedAssessmentWorkflowState(instanceId);
      setState(data);
    } catch (e: any) {
      setError(e.message ?? 'Failed to load assessment state');
    }
  }, [instanceId]);
  ...
  useEffect(() => {
    ...
    async function initialize() {
      ...
      try {
        ...
        if (cancelled) return;
        setState(data);
        const defs = data.assessment_definition.stage_defs ?? [];
  ...
  const handleApproveFinal = useCallback(async () => {
    ...
    try {
      const result = await api.approveFinalAssessmentOutput(instanceId, state?.workflow_version);
      setState((prev) => prev ? {
        ...prev,
        workflow_state: result.workflow_state,
        workflow_version: result.workflow_version,
  ...
  const handleRevokeFinalApproval = useCallback(async () => {
    setIsApprovingFinal(true);
    try {
      const result = await api.revokeFinalAssessmentApproval(instanceId, state?.workflow_version);
      setState((prev) => prev ? {
        ...prev,
        workflow_state: result.workflow_state,
        workflow_version: result.workflow_version,
  ...
  const { workflow_state: ws, assessment_definition: mod } = state;
  ...
  if (!currentStageDef) {
    return <div className="p-4 text-sm text-text-secondary">No stages configured for this assessment.</div>;
  }
  ...
  const canApproveFinal = requiresFinalApproval
    ...
    && terminalStageReady;
  const canExportAssessment = hasExport
    && stageDefs.length > 0
    && stagesBeforeTerminalConfirmed;
          ...
          <div className={isAssessmentMapWidget ? 'mt-[38px] flex min-h-0 flex-1 flex-col' : 'mt-[38px] max-w-3xl mx-auto w-full flex flex-col'}>
            ...
            {isComputedStage ? (
              ...
              <div className="card overflow-hidden">
                ...
                <div className={isEditableInputTableStage || isCalculationComputedWidget ? '' : 'p-4'}>
                  {renderStageContent()}
                </div>
