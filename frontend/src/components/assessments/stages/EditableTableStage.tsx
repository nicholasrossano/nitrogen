...
interface StageTableRow extends ModelInputRow {
  itemId: string;
  ...
  if (fieldType === 'select' && options?.length) {
    return (
      <CustomDropdown
        ...
        options={[
          { value: '', label: '—' },
          ...options.map((o) => ({ value: o, label: o.replace(/_/g, ' ') })),
        ]}
        ariaLabel="Select value"
        className="h-7 w-28 inline-flex items-center justify-between gap-1 rounded border border-accent/40 bg-surface px-1.5 text-xs text-text-primary font-mono focus:outline-none focus:ring-1 focus:ring-accent"
        menuClassName="absolute right-0 top-full z-50 mt-1 min-w-[140px] rounded-md border border-stroke-subtle bg-white p-1 shadow-lg"
...
export function EditableTableStage({
  ...
  const proposalModelType = PROPOSAL_MODEL_TYPES_BY_MODULE_ID[assessmentId] ?? null;
  const enableInvestigate = !!proposalModelType && stageId === 'inputs' && !interactionsDisabled;
  const isSolarInputsStage = assessmentId === SOLAR_LOCATION_MODULE_ID && stageId === 'inputs';
  ...
  const groupedTableRows = useMemo(() => {
    ...
    return groupedItems.map((group) => ({
      category: group.cat,
      label: group.label,
      inputs: group.rows.map((item) => {
        const content = item.content ?? {};
        const variableLabel = String(content[nameField?.name ?? 'variable'] ?? '');
        const unit = unitField ? String(content[unitField.name] ?? '') : '';
        const status = String(content.status ?? (item.origin === 'inferred' ? 'extracted' : 'missing'));
        const explicitFieldName = typeof content.field_name === 'string' ? content.field_name : '';
        const fieldName = explicitFieldName || normalizeKey(variableLabel || item.id);
        const valueField = fields.find((f) => f.name === 'value') ?? fields[1] ?? fields[0];
        const fieldType = String(content.field_type ?? (valueField?.field_type ?? 'text'));
        ...
        return {
          ...
          value: valueField ? (content[valueField.name] ?? null) : null,
          unit: unit || null,
          status,
          rationale: typeof content.rationale === 'string' ? content.rationale : null,
          valueFieldName: valueField?.name ?? 'value',
          fieldType,
          options,
  ...
  }, [groupedItems, fields, interactionsDisabled, deleting]);
          ...
          renderValueCell={(row) => (
            row.readOnly ? (
              ...
              <ValueEditor
                ...
                options={row.options}
                onSave={(value) => handleSave(row.itemId, row.valueFieldName, value)}
              />
            )
          )}
        />
