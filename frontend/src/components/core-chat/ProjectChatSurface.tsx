...
export function ProjectChatSurface({
  ...
  const [activeAssessmentsCount, setActiveAssessmentsCount] = useState<number | null>(
    () => activeAssessmentsCountCache.get(projectId) ?? null,
  );
  ...
  useEffect(() => {
    ...
    const cachedCount = activeAssessmentsCountCache.get(projectId);
    ...
    api.listAssessmentInstances(projectId)
      .then((instances) => {
        if (!cancelled) {
          activeAssessmentsCountCache.set(projectId, instances.length);
  ...
  }, [projectId, isOnLanding, isOverviewLanding]);
  ...
  }, [generateProjectOverview, project, projectId]);

  const handleRefreshOverviewPanel = useCallback(() => {
    if (!project || project.shared_role === 'viewer') return;
    if (projectMaterials.length > 0) void handleGenerateOverview();
    setHealthRefreshToken((prev) => prev + 1);
  }, [handleGenerateOverview, project, projectMaterials.length]);

  useEffect(() => {
    setOverviewError(null);
    setOverviewGenerating(false);
    setActiveAssessmentsCount(activeAssessmentsCountCache.get(projectId) ?? null);
    setHealthRefreshToken(0);
    autoOverviewAttemptRef.current = null;
  }, [projectId]);

  useEffect(() => {
    if (!isOverviewLanding || !project || project.shared_role === 'viewer') return;
    if (projectMaterials.length === 0) return;
    if (project.overview_description?.trim()) return;
    if (overviewGenerating) return;

    const attemptKey = `${projectId}:${projectMaterials.length}`;
    ...
    return (
      <div className="h-full min-h-0">
        <LandingInput
          ...
          topRightActions={isOverviewLanding && hideTiles && !landingHeaderContent && project ? (
            <>
              <button
                type="button"
                onClick={handleRefreshOverviewPanel}
                disabled={project.shared_role === 'viewer' || overviewGenerating}
