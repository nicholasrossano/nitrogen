...
type EnrichedInstance = AssessmentInstance & { index: number | null; displayName: string };

function enrich(instances: AssessmentInstance[]): EnrichedInstance[] {
  return instances.map((inst) => {
    if (inst.display_name) {
      return { ...inst, index: inst.instance_number ?? null, displayName: inst.display_name };
    }

    const sameToolInstances = instances.filter((i) => i.assessment_id === inst.assessment_id);
    ...
    const index = hasDuplicates
      ...
      : null;
    const mod = MODULE_MAP.get(inst.assessment_id);
    const displayName = mod
      ? index ? `${mod.name} #${index}` : mod.name
      : index
        ? `${inst.assessment_id.replace(/_/g, ' ')} #${index}`
        : inst.assessment_id.replace(/_/g, ' ');
...
export function OpenAssessmentBrowser({ projectId, onSelect }: OpenAssessmentBrowserProps) {
  ...
  const visibleInstances = useMemo(
    () => instances.filter((instance) => {
      const assessmentOption = MODULE_MAP.get(instance.assessment_id);
      if (!assessmentOption) return true;
      return showBetaAssessments || !assessmentOption.beta;
    }),
    [showBetaAssessments, instances],
  );
      ...
      <div className="w-full pt-3 pb-8">
        <div className="mb-6 flex items-center justify-end gap-4">
          ...
          >
            ...
            {isTrashView ? 'Back to Assessments' : 'Trash'}
          </button>
        ...
        {loading ? (
          ...
          <div className="py-12 text-center">
            <p className="text-sm text-text-secondary">
              {isTrashView ? 'Trash is empty.' : 'No assessments started yet.'}
            </p>
            {!isTrashView && (
              <>
