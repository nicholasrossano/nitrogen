...
interface AssessmentPickerProps {
  ...
  onSelect: (assessment: AssessmentOption | null) => void;
  disabled?: boolean;
  /** 'project' shows all assessments; 'standalone' shows only analysis assessments */
...
export function AssessmentPicker({
  ...
  const assessments = useVisibleAssessments(mode === 'standalone' ? ANALYSIS_MODULES : ALL_MODULES);
  ...
  const handleSelect = (assessment: AssessmentOption) => {
    onSelect(selected?.id === assessment.id ? null : assessment);
    setOpen(false);
  };
    ...
    >
      ...
      <div className="pb-1.5">
        {assessments.map((assessment) => {
          const isActive = selected?.id === assessment.id;
          return (
            <button
              key={assessment.id}
              type="button"
  ...
  return (
    <>
      <button
        ...
        aria-label={selected ? `Assessment: ${selected.name}` : 'Select a assessment'}
      >
...
/** Dismissable chip displayed above the textarea when a assessment is selected */
