"""Base classes for Nitrogen assessment system."""
...
class AssessmentInput:
    """Definition of an input field for a assessment (used in chat path)."""
    name: str
    label: str
    description: str
...
class AssessmentOutput:
    """Result from running a assessment."""
    assessment_id: str
    output_type: str
    title: str
...
class PopulationStep:
    ...
    Supported types:
        start_from_predefined_rows      — assessment.get_predefined_rows(stage_id, context)
        seed_from_template              — assessment.generate_items_for_stage(stage_id, step_type, …)
        ...
        compute_with_assessment_logic       — assessment.compute_stage(stage_id, confirmed_stages, ctx)
        compute_with_external_tool      — assessment.compute_external(stage_id, tool, confirmed, ctx)
...
@dataclass
class StageDef:
    """Definition of a single stage in a assessment's workflow.

    Every assessment is an ordered sequence of stages. Each stage is a
...
class DecisionLogAttribution:
    """Schema-backed decision-log attribution preferences for a assessment."""
...
@dataclass(kw_only=True)
class AssessmentManifest(AssessmentDefinition):
    """Full assessment contract metadata used for registry and exposure layers."""
    goal: str
...
class BaseAssessment(ABC):
    """Abstract base class for all Nitrogen assessments.

    A assessment is an ordered sequence of stages, each of which is a
    confirmable workspace. The assessment declares its stages via stage_defs
    ...
    def manifest(self) -> AssessmentManifest:
        """Return the full assessment manifest."""
        raise NotImplementedError

    @property
    @abstractmethod
    def stage_defs(self) -> list[StageDef]:
        """Ordered stage definitions for this assessment's workflow."""
        pass

    # ------------------------------------------------------------------ #
    # Population hooks                                                     #
    # These are called by the population step executor in the workflow     #
    # service. Override only the hooks your assessment needs.                  #
    # ------------------------------------------------------------------ #
    ...
    async def compute_stage(
        ...
        """Run assessment-specific computation for 'compute_with_assessment_logic'.
    ...
    # These are used when assessments are invoked from conversation context.   #
    # ------------------------------------------------------------------ #
    ...
    @property
    def optional_inputs(self) -> list[AssessmentInput]:
        return []

    @property
    def all_inputs(self) -> list[AssessmentInput]:
    ...
    async def execute_from_conversation(
        ...
        """Execute assessment from conversation text (chat path only).

        Returns (widget_type, widget_data). Override in calculator assessments
