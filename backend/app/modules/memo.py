"""memo tool - generates structured investment recommendation documents."""
...
class MemoTool(BaseModule):
    """Tool for generating memos with RAG-grounded citations."""
    ...
    def manifest(self) -> ModuleManifest:
        return ModuleManifest(
            **self.definition.__dict__,
            ...
            goal="Produce an memo with recommendation and evidence-backed rationale.",
...
"""
        ...
        return ModuleAlignment(
            ...
            title=f"memo Outline: {project_title}",
    ...
    async def _generate_outline(self, project_summary: str) -> dict:
        ...
        system_prompt = """You are an expert at structuring memos for development projects.
...
"""
    ...
    async def update_alignment_from_feedback(
        ...
        system_prompt = """You are refining an memo outline based on user feedback.
...
"""
    ...
    async def execute(
        ...
        """Generate the memo, optionally using alignment configuration."""
        from sqlalchemy import select
        from app.models.initiative import Initiative
        ...
        if alignment is None:
            ...
            memo_title = memo_payload.get("title") or f"memo: {inputs.get('project_title', 'Untitled Project')}"
...
"""
        ...
        if "sections" in memo_data:
            ...
            memo_content = {
                "title": f"memo: {inputs.get('project_title', 'Untitled Project')}",
...
- Clearly state when claims are based on general knowledge vs. specific evidence."""
        ...
        base_system_prompt = f"""You are an expert analyst generating memos for development initiatives.
...
- Concise but thorough"""
        ...
        user_message = f"""Generate an memo for this project.
        ...
        if alignment and alignment.sections:
            ...
            response = await client.chat.completions.create(
                ...
                tools=[{
                    ...
                    "function": {
                        "name": "generate_memo",
                        "description": "Generate structured memo with custom sections",
                        "parameters": {
                            "type": "object",
                            "properties": {
                                "recommendation": {
            ...
            response = await client.chat.completions.create(
                ...
                tools=[{
                    ...
                    "function": {
                        "name": "generate_memo",
                        "description": "Generate structured memo",
                        "parameters": {
                            "type": "object",
                            "properties": {
                                "executive_summary": {
