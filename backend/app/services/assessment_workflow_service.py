"""Unified staged assessment workflow service.

Every assessment is an ordered sequence of stages. Each stage is a confirmable
workspace. The workflow state stored in assessment_instances.workflow_state is:
...
def requires_final_approval(assessment: BaseAssessment) -> bool:
    """Whether a workspace assessment should require explicit final approval."""
    return bool(assessment.stage_defs)
...
def build_deliverable_title(assessment: BaseAssessment, content: dict[str, Any] | None) -> str:
...
def _build_initial_workflow_state(assessment: BaseAssessment) -> dict[str, Any]:
    """Build the initial workflow state dict from the assessment's stage_defs."""
...
def _infer_current_stage_id(assessment: BaseAssessment, stages: dict[str, Any]) -> str | None:
...
def _migrate_legacy_state(old_state: dict[str, Any], assessment: BaseAssessment) -> dict[str, Any]:
    ...
    Handles four legacy shapes:
      ...
      3. build.widget_state{}:    single widget blob (calculator assessments)
      4. output.widget_data:      top-level output blob (calculator assessments)
    """
    new_state = _build_initial_workflow_state(assessment)
    ...
    # layers{} format (old assessment assessments)
    ...
    # widget_state blob (old calculator assessments stored results in build.widget_state)
    ...
    new_state["current_stage_id"] = _infer_current_stage_id(assessment, stages)
    logger.info(
        "Migrated legacy workflow state for assessment '%s'", assessment.definition.id
    )
    return new_state
...
def _invalidate_downstream(
    ...
    for stage_id in _get_downstream_stage_ids(assessment, changed_stage_id):
...
def _hydrate_state(inst: AssessmentInstance, assessment: BaseAssessment) -> dict[str, Any]:
    """Return a deepcopy of workflow_state with any missing stage keys added.

    This ensures that stages added to a assessment after an instance was created
    ...
    # Add any stages defined in the assessment that aren't in the stored state
...
async def populate_stage(
    ...
    if stage_id not in state["stages"]:
        raise ValueError(f"Stage '{stage_id}' not found in assessment '{assessment.definition.id}'")
...
async def _execute_population_step(
    ...
    if step_type == "start_from_predefined_rows":
        rows = await assessment.get_predefined_rows(stage_def.id, context)
        items = [
    ...
    if step_type == "seed_from_template":
        raw_items = await assessment.generate_items_for_stage(
    ...
    if step_type == "extract_from_project_materials":
        ...
        # For now this is a no-op that passes through; assessments that need RAG
    ...
    if step_type == "infer_missing_with_ai":
        ...
        # Assessment assessments handle this within generate_items_for_stage.
        # For calculator assessments this is also handled in get_predefined_rows
    ...
    if step_type == "enrich_selected_item_with_ai":
        if config.get("bulk") and stage_def.component == "record":
            ...
            existing_records = accumulated_data.get("records", {})
            bulk_enricher = getattr(assessment, "enrich_records_for_stage", None)
    ...
    if step_type == "compute_with_assessment_logic":
        widget_data = await assessment.compute_stage(stage_def.id, confirmed_stages, context)
        return {"widget_data": widget_data}

    if step_type == "compute_with_external_tool":
        tool = config.get("tool", "")
        widget_data = await assessment.compute_external(stage_def.id, tool, confirmed_stages, context)
        return {"widget_data": widget_data}
...
async def confirm_stage(
    ...
    _invalidate_downstream(state, assessment, stage_id)
    ...
    stage_def_being_confirmed = next(
        (s for s in assessment.stage_defs if s.id == stage_id), None
    )
    ...
    state["current_stage_id"] = _infer_current_stage_id(assessment, state["stages"])
    clear_final_approval(state)

    # Trigger auto-population of the next computed_results stage, if any
    next_stage_def = _get_auto_populate_stage(assessment, stage_id, state)
...
async def enrich_record_item(
    ...
    state = inst.workflow_state or _build_initial_workflow_state(assessment)
    if _is_legacy_state(state):
        state = _migrate_legacy_state(state, assessment)
...
def uses_workspace_flow(assessment: BaseAssessment) -> bool:
    """Whether the assessment can open inside the workspace flow."""
    return bool(assessment.stage_defs)


def is_calculator_assessment(assessment: BaseAssessment) -> bool:
    """Whether any stage uses compute_with_assessment_logic or compute_with_external_tool."""
    for stage_def in assessment.stage_defs:
        for step in stage_def.population:
            if step.type in ("compute_with_assessment_logic", "compute_with_external_tool"):
                return True
    return False


def is_structured_assessment(assessment: BaseAssessment) -> bool:
