"""Resolve incoming assumptions onto canonical project rows (exact / alias / fuzzy).
...
def _surface_forms(row: Assumption) -> list[str]:
...
def _config_alias_keys() -> dict[str, str]:
    ...
    for definition in ASSUMPTION_BY_KEY.values():
        mapping[normalize_assumption_key(definition.key)] = definition.key
        mapping[normalize_assumption_key(definition.label)] = definition.key
        for alias in definition.aliases:
            mapping[normalize_assumption_key(alias)] = definition.key
    return mapping
...
async def resolve_canonical_assumption(
    db: AsyncSession,
    project_id: UUID,
    *,
    key: str,
    label: str | None = None,
    existing_rows: Iterable[Assumption] | None = None,
) -> Assumption | None:
    """Return the best matching project assumption, or None if none match safely."""
    ...
    # 1) Exact key
    for row in rows:
        if normalize_assumption_key(row.key) == normalized_key:
            return row

    # 2) Config alias seed -> exact row keyed by canonical key
    config_canonical = _CONFIG_ALIAS_KEYS.get(normalized_key)
    if label_text:
        config_canonical = config_canonical or _CONFIG_ALIAS_KEYS.get(normalize_assumption_key(label_text))
    if config_canonical:
        for row in rows:
            if normalize_assumption_key(row.key) == normalize_assumption_key(config_canonical):
                return row

    # 3) Fuzzy label / aliases (high threshold)
    candidates: list[tuple[float, Assumption]] = []
