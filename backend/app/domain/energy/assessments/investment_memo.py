...
MEMO_SYSTEM_RULES = (
    "You are an expert analyst writing investment memos for development initiatives "
...
class InvestmentMemoAssessment(BaseAssessment):
    """Investment Memo — outline confirmation then cited, project-consistent writeup."""
    ...
    def definition(self) -> AssessmentDefinition:
        return AssessmentDefinition(
            ...
            description=(
                "Structured investment memo with confirmed outline, recommendation, "
    ...
    def manifest(self) -> AssessmentManifest:
        return AssessmentManifest(
            **self.definition.__dict__,
            goal=(
                "Produce an investment memo consistent with approved project assessments, "
    ...
    async def generate_writeup_content(
        ...
        *,
        ...
        if not result.get("title"):
            ...
            result["title"] = f"Investment Memo — {title}"
    ...
    async def _generate_outline(self, context: dict) -> list[dict]:
        ...
        data = await llm_json(
            system=(
                "You are an expert at structuring investment memos for development finance. "
