"""API endpoints for the unified staged assessment workflow.

Every assessment is an ordered sequence of confirmable stages. These endpoints
drive the AssessmentWorkspace frontend component. Each endpoint operates on a
AssessmentInstance identified by its UUID.

Mounted at: /api/v1/assessment-workflow
"""
...
async def _get_workflow_instance(
    ...
    user: AuthUser,
) -> tuple[AssessmentInstance, BaseAssessment]:
    ...
    if assessment is None or not uses_workspace_flow(assessment):
        raise HTTPException(
            status_code=400,
            detail=f"Assessment '{inst.assessment_id}' is not configured for workspace flow",
        )
...
def _assessment_definition_payload(assessment: BaseAssessment) -> dict[str, Any]:
    """Build the assessment definition payload returned with workflow state."""
    return {
        "id": assessment.definition.id,
        "name": assessment.definition.name,
        "icon": assessment.definition.icon,
        "output_type": assessment.definition.output_type,
        "export_format": assessment.definition.export_format,
        "requires_final_approval": requires_final_approval(assessment),
        "stage_defs": [s.to_dict() for s in assessment.stage_defs],
    }


async def _instance_creator_token(db: AsyncSession, inst: AssessmentInstance) -> str:
...
async def _assessment_export_filename(
    *,
    db: AsyncSession,
    ...
    """Build a consistent filename for assessment-scoped exports."""
...
async def _generate_legacy_calculator_export(
    ...
    """Reuse existing calculator workbook exports for staged assessment workflows."""
    ...
    raise RuntimeError(f"No legacy export fallback is defined for assessment '{assessment_id}'")
...
def _assert_workflow_version(inst: AssessmentInstance, request: Request) -> None:
    ...
    if expected != current:
        raise HTTPException(
            status_code=409,
            detail={
                "message": "This assessment was updated elsewhere. Refresh and try again.",
                "current_workflow_version": current,
            },
        )


def _all_required_stages_confirmed(assessment: BaseAssessment, state: dict[str, Any]) -> bool:
...
def _get_auto_confirmable_final_stage(assessment: BaseAssessment, state: dict[str, Any]) -> tuple[str, dict[str, Any]] | None:
    """Return the terminal stage when it is ready to auto-confirm during final approval."""
    if not assessment.stage_defs:
        return None

    final_stage = assessment.stage_defs[-1]
    ...
    prior_stage_defs = assessment.stage_defs[:-1]
    if not all(
        state["stages"].get(stage_def.id, {}).get("status") == "confirmed"
...
def _get_stage_def_or_404(
    ...
    stage_def = next((s for s in assessment.stage_defs if s.id == stage_id), None)
    if stage_def is None:
        raise HTTPException(
            status_code=404,
            detail=f"Stage '{stage_id}' not defined in assessment for instance {instance_id}",
        )
    return stage_def
...
async def _refresh_stakeholder_map_widget(
    ...
    widget_data = await assessment.compute_stage("map", confirmed_stages, context)
...
@router.get("/assessment-workflow/{instance_id}/state")
async def get_workflow_state(
    instance_id: _uuid.UUID,
    db: AsyncSession = Depends(get_db),
    user: AuthUser = Depends(get_current_user),
):
    """Return the full workflow state for a assessment instance plus its assessment definition."""
...
@router.post("/assessment-workflow/{instance_id}/stages/{stage_id}/populate")
...
@router.post("/assessment-workflow/{instance_id}/stages/{stage_id}/confirm")
...
@router.patch("/assessment-workflow/{instance_id}/stages/{stage_id}/items/{item_id}")
async def edit_item(
    instance_id: _uuid.UUID,
    stage_id: str,
    item_id: str,
...
@router.post("/assessment-workflow/{instance_id}/stages/{stage_id}/items")
async def add_item(
    instance_id: _uuid.UUID,
    stage_id: str,
    data: AddItemRequest,
...
@router.delete("/assessment-workflow/{instance_id}/stages/{stage_id}/items/{item_id}")
...
@router.post("/assessment-workflow/{instance_id}/stages/{stage_id}/reorder")
...
@router.post("/assessment-workflow/{instance_id}/stages/{stage_id}/records/{item_id}/enrich")
async def enrich_record_endpoint(
    ...
    Calls assessment.enrich_record() and persists the enriched field values.
    """
    inst, assessment = await _get_editable_workflow_instance(db, instance_id, user)
    _assert_workflow_version(inst, request)
    prior_approved = ((inst.workflow_state or {}).get("final_approval") or {}).get("status") == "approved"

    enriched = await enrich_record_item(db, inst, assessment, stage_id, item_id)
    await append_decision_event(
        db,
        inst=inst,
...
@router.post("/assessment-workflow/{instance_id}/stakeholders/{item_id}/enrich")
async def enrich_stakeholder_from_map(
    ...
    inst, assessment = await _get_editable_workflow_instance(db, instance_id, user)
    if not isinstance(assessment, StakeholderAssessment):
...
@router.post("/assessment-workflow/{instance_id}/implementation/{item_id}/deep-dive")
async def deep_dive_implementation_item(
    ...
    inst, assessment = await _get_editable_workflow_instance(db, instance_id, user)
    if not isinstance(assessment, ImplementationPlanAssessment):
...
@router.patch("/assessment-workflow/{instance_id}/stages/{stage_id}/records/{item_id}")
...
# SolarEstimateWidget call persistAssessmentWorkflowWidget() when the user edits
...
@router.post("/assessment-workflow/{instance_id}/widget-state")
async def persist_widget_state(
    ...
    api.persistAssessmentWorkflowWidget() after an inline edit.
    """
    inst, assessment = await _get_editable_workflow_instance(db, instance_id, user)
    _assert_workflow_version(inst, request)
    state = await ensure_workflow_state(db, inst, assessment)
    ...
    target_stage_def = next(
        (s for s in assessment.stage_defs if s.component == "computed_results"),
        None,
    )
    if target_stage_def is None:
        raise HTTPException(
            status_code=400,
            detail="This assessment has no computed_results stage to write widget data to",
        )
...
@router.post("/assessment-workflow/{instance_id}/final-approval")
async def approve_final_output(
    ...
    state = await ensure_workflow_state(db, inst, assessment)
    auto_confirm_candidate = _get_auto_confirmable_final_stage(assessment, state)
    ...
    if not _all_required_stages_confirmed(assessment, state):
        raise HTTPException(
            status_code=400,
...
@router.delete("/assessment-workflow/{instance_id}/final-approval")
...
@router.get("/assessment-workflow/{instance_id}/export")
async def export_assessment_output(
    instance_id: _uuid.UUID,
    db: AsyncSession = Depends(get_db),
    user: AuthUser = Depends(get_current_user),
):
    ...
    if assessment.definition.export_format is None:
    ...
    filename = await _assessment_export_filename(db=db, inst=inst, assessment=assessment, ext=fmt)
    await append_decision_event(
        db,
        inst=inst,
        event_type="exported",
        ...
        actor_user_id=user.uid,
        actor_email=user.email,
        payload={"format": fmt, "scope": "assessment"},
    )
    await db.commit()
...
async def export_writeup(
    ...
    filename = await _assessment_export_filename(
        db=db,
        inst=inst,
...
@router.get("/assessment-workflow/{instance_id}/decision-log/export.xlsx")
async def export_assessment_decision_log_xlsx(
    instance_id: _uuid.UUID,
    db: AsyncSession = Depends(get_db),
    user: AuthUser = Depends(get_current_user),
):
    ...
    await append_decision_event(
        ...
        actor_user_id=user.uid,
        actor_email=user.email,
        payload={"format": "xlsx", "scope": "assessment"},
    )
    await db.commit()
