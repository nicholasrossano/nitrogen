...
async def test_lcoe_compute_stage_preserves_input_status_and_source(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(assessment, "recalculate_from_values", fake_recalculate_from_values)
    ...
    widget_data = await assessment.compute_stage("results", confirmed_stages, {})
...
async def test_pvwatts_compute_external_preserves_input_status_and_source(
    ...
    widget_data = await assessment.compute_external("results", "pvwatts", confirmed_stages, {})
