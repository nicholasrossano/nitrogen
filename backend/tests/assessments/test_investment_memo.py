"""Tests for the Investment Memo staged assessment."""

import pytest

from app.assessments import get_assessment_registry
from app.assessments.utils import make_build_item
from app.domain.energy.assessments.investment_memo import (
    DEFAULT_MEMO_SECTIONS,
    InvestmentMemoAssessment,
)


def test_investment_memo_registered_and_stage_flow():
    registry = get_assessment_registry()
    assessment = registry.get_assessment("investment_memo")
    assert assessment is not None
    assert isinstance(assessment, InvestmentMemoAssessment)
    ...
    assert assessment.manifest.produced_outputs == ["investment_memo", "memo_citations"]


@pytest.mark.asyncio
async def test_generate_outline_falls_back_to_defaults(monkeypatch):
    assessment = InvestmentMemoAssessment()
    ...
    monkeypatch.setattr(
        "app.domain.energy.assessments.investment_memo.llm_json",
        fake_llm_json,
    )
