...
def test_risk_assessment_stage_defs_follow_expected_flow():
    ...
    assert module.definition.id == "risk_assessment"
    assert module.definition.export_format == "xlsx"
    assert [stage.id for stage in module.stage_defs] == [
        "categories",
        "risks",
        "mitigations",
        "results",
    ]
    assert module.stage_defs[-1].title == "Results"
    assert module.stage_defs[-1].widget == "risk_register_results"
    assert module.manifest.adapter_bindings == {"research_source": "retrieval"}
    assert module.manifest.produced_outputs == ["risk_register"]
...
async def test_compute_stage_builds_register_and_normalizes_ratings(monkeypatch):
    ...
    monkeypatch.setattr(module, "_rate_register", fake_rate_register)

    result = await module.compute_stage(
        "results",
        {
...
def test_export_xlsx_includes_register_sheets():
    module = RiskAssessmentModule()
    workbook_bytes = module.export_xlsx({
        "risk_register": [{
            "risk_id": "R01",
...
async def test_generate_risks_falls_back_to_concrete_category_specific_rows(monkeypatch):
    ...
    risks = await module._generate_risks(
        {
            "project_type": "energy_access",
            "geography": "Kenya",
