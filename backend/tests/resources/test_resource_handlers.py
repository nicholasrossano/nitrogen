...
@pytest.mark.parametrize(
    ...
    [
        (
            "_read_evidence_doc",
            f"nitrogen://projects/{uuid4()}/evidence/docs/{uuid4()}",
            "EvidenceDoc not found.",
        ),
        (
            "_read_evidence_chunk",
            f"nitrogen://projects/{uuid4()}/evidence/chunks/{uuid4()}",
            "EvidenceChunk not found.",
        ),
        (
            "_read_project_material",
            f"nitrogen://projects/{uuid4()}/materials/{uuid4()}",
        ...
        (
            "_read_artifact",
            f"nitrogen://projects/{uuid4()}/artifacts/{uuid4()}",
            "AssessmentInstance not found.",
        ),
    ],
)
async def test_project_scoped_handlers_parse_valid_uri_before_db_lookup(
    ...
    monkeypatch.setattr(resource_handlers, "_ensure_project_access", _allow_access)
